...
string minWindow(string s, string t) {
  int charLastIndex[26];         // 保存每个字符最后一次出现的位置
  int validHead;                 // 有效字符串的头字母(-'A')
  ...
  for (int i = 0; i < t.length(); i++)
    charLastIndex[t[i] - 'A'] = -1;

  int i;
  // 先找到最左边的有效字符串
  for (i = 0; i < s.length() && sumAppearChar != t.length(); i++) {
    // 无关字符处理
    if (charLastIndex[s[i]- 'A'] == s.length())  continue;

    // 第一次出现
    if (charLastIndex[s[i]- 'A'] == -1)
      sumAppearChar++;
  ...
  for (; i < s.length(); i++) {
    ...
    if (s[i] - 'A' == validHead) {
      ...
      // for (int i = 0; i < 26; i++)  cout << charLastIndex[i] << ' ';
      // cout << endl;
